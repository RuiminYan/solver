...
// Search 1: XC �?+ EC �?
STAT_DECL(s1_prune1); // S1: XC 剪枝�?
STAT_DECL(s1_edge);   // S1: EC 边剪枝表

// Search 2: Base + XC
STAT_DECL(s2_prune1); // S2: Base 剪枝�?
STAT_DECL(s2_edge);   // S2: Edge 剪枝�?
// NOTE: s2_prune2 已移�?(剪枝�?0%)
STAT_DECL(s2_xc2); // S2: XC2 剪枝�?

// Search 3: Aux + XC
STAT_DECL(s3_aux);    // S3: 辅助�?(Corner2/Edge2)
STAT_DECL(s3_prune1); // S3: 基础剪枝�?
STAT_DECL(s3_edge);   // S3: Edge 剪枝�?
STAT_DECL(s3_xc3);    // S3: XC3 剪枝�?

// Search 4: Aux + Multi
STAT_DECL(s4_aux);    // S4: 辅助�?(Corner3/Edge3)
STAT_DECL(s4_prune1); // S4: 基础剪枝�?1
STAT_DECL(s4_edge);   // S4: Edge 剪枝�?
STAT_DECL(s4_prune2); // S4: 基础剪枝�?2
STAT_DECL(s4_prune3); // S4: 基础剪枝�?3
STAT_DECL(s4_prune4); // S4: 基础剪枝�?4
STAT_DECL(s4_xc4);    // S4: XC4 剪枝�?
...
struct xcross_analyzer2 {
  ...
  static const int *p_edge3_move_ptr;   // Edge3 移动表指�?
  static const int *p_corner3_move_ptr; // [新增] Corner3 移动表指�?
  static const int *p_corner2_move_ptr; // [新增] Corner2 移动表指�?
  static const int *p_edge2_move_ptr;   // [新增] Edge2 移动表指�?

  // [重构] Aux 剪枝表指�?(�?Manager 获取)
  // Edge3 剪枝�?
  static const unsigned char *p_aux_e3_prune; // E0E1E2 规范�?

  // Corner3 剪枝�?
  static const unsigned char *p_aux_c3_prune; // C4C5C6 规范�?
  ...
  struct AuxPrunerDef {
    ...
    const int *p_move;            // 移动表指�?
    int multiplier;               // 状态乘�?(Corner3=9072, Edge3=10560)
  };

  struct AuxState {
    const AuxPrunerDef *def = nullptr;
    int current_idx = 0;
    int current_cross_scaled = 0;     // 虚拟 Cross 状�?* 24
    const int *move_mapper = nullptr; // rot_map[rot_idx]
    int slot_k = 0;                   // 共轭参考槽�?(pslot1)
  };

  // [Conj优化] XC 预计算状�?(用于 pseudo_base 表的 Conj 优化)
  // �?pslot 视角追踪 Cross + Corner + 4个Edge 状�?
  struct ConjStateXC {
    int cross;   // Cross 索引 (�?×24 缩放)
    int corner;  // Corner 索引 (�?C4=12 开�?
    int edge[4]; // 4 �?Edge 相对索引 (�?{0,2,4,6} 开�?
  };

  static constexpr int MAX_AUX = 8;

  // === 静�?AuxPrunerDef 对象 (替代 aux_registry map) ===
  // Edge2: 邻接 (E0E1) �?对角 (E0E2)
  static AuxPrunerDef aux_def_e2_adj;  // {0,1}: prune_e0e1
  static AuxPrunerDef aux_def_e2_diag; // {0,2}: prune_e0e2
  // Corner2: 邻接 (C4C5) �?对角 (C4C6)
  ...
  static inline int get_e2_type(int e1, int e2) {
    int diff = (e2 - e1 + 4) & 3;
    return (diff == 2) ? 1 : 0; // diff==2 表示对角 ({0,2} �?{1,3})
  }
  // Corner2: 返回 0=邻接, 1=对角
  static inline int get_c2_type(int c1, int c2) {
    int diff = (c2 - c1 + 4) & 3;
    return (diff == 2) ? 1 : 0; // diff==2 表示对角 ({4,6} �?{5,7})
  }

  // === 获取 AuxPrunerDef 的辅助函�?===
  // 根据规范�?keys 返回对应�?AuxPrunerDef 指针
  ...
  static void initialize_tables() {
    ...
    // [重构] �?PruneTableManager 统一加载 PseudoPair �?
    ...
    // 复用 Pseudo Analyzer �?pt_pscross_C4E
    for (int e = 0; e < 4; ++e) {
      p_pseudo_base_prune[e] = ptm.getPsCrossC4EPTPtr(e);
    }

    // 5. 获取 Aux 表指�?(�?Manager，已�?loadPseudoPairTables 加载)
    // Edge3 移动�?
    mtm.loadMTEdge3();
  ...
  int setup_aux_pruners_for_search4(int pslot1_arg, // 参考槽�?(slot_k)
    ...
    // 计算相对槽位 (相对�?slot_k)
    {
      ...
      const AuxPrunerDef *def_ptr = &aux_def_c3;
      {
        ...
        for (int m : alg) {
          ...
          int m_rot = mapper[m_conj];              // slot_k �?规范�?
    ...
    // 计算相对槽位 (相对�?slot_k)
    {
      int r1 = (slot2_arg - slot_k + 4) % 4; // 映射�?0-3 范围
  ...
  // 使用两级映射：conj_moves_flat[m][slot_k] + rot_map[rot_idx]
  int setup_aux_pruners_for_search3(int pslot1_arg, // 参考槽�?(slot_k)
                                    int slot2_arg,
                                    int slot3_arg, // Edge 槽位 (固定�?
                                    int pslot2_arg,
                                    int pslot3_arg, // Corner 伪槽�?
                                    const std::vector<int> &alg,
                                    AuxState *out_aux) {
    int count = 0;
    int slot_k = pslot1_arg; // 参考槽�?
  ...
  // [Conj优化] XC2 使用 Conj 状态追�?
  // 新增参数: xc2_cross, xc2_corner, xc2_e0-e3 (Conj 状�?, diff2 (边选择)
  bool search_2(int arg_index1, int arg_index2, int arg_index4, int arg_index5,
    ...
    for (int k = 0; k < count_moves; ++k) {
      ...
      // 1. [Conj] prune_xc2 剪枝 (77.57% 剪枝率，最高优�?
  ...
  // [Conj优化] XC3 使用 Conj 状态追�?
  // NOTE: prune2/prune3 已移�?(剪枝�?0%, �?AuxState 完全覆盖)
  bool search_3(int arg_index1, int arg_index2, int arg_index7, int arg_index8,
    ...
    for (int k = 0; k < count_moves; ++k) {
      ...
      // 1. 首先计算 Cross 状态索�?(用于 Aux 剪枝)
      int index1_tmp = p_multi_move_ptr[arg_index1 + i];
      int cross_state_idx = index1_tmp / 24;

      // 2. Aux Pruning (最�? Corner2 �?Edge2)
      ...
      for (int a = 0; a < num_aux; ++a) {
        ...
        if (cur.move_mapper) {
          ...
          int m_rot = cur.move_mapper[m_conj];         // slot_k �?规范�?
      ...
      // ++s3_aux_checked;  // NOTE: S3统计已禁�?
      if (aux_pruned) {
        // ++s3_aux_pruned;
        continue;
      }

      // 2. [Conj] prune_xc3 剪枝 (26.16% 剪枝率，优先级高�?prune1)
  ...
  // [Conj优化] XC4 使用 Conj 状态追�?
  // 新增参数: xc4_cr/cn/e0-e3 (Conj 状�?, diff4 (边选择)
  // Search 4
  bool search_4(int arg_index1, int arg_index2, int arg_index4, int arg_index6,
    ...
    for (int k = 0; k < count_moves; ++k) {
      ...
      // [重构] AuxState 剪枝 (Corner3 + Edge3, 优先于其他剪�?)
      // ++s4_aux_checked; // 统计已禁�?
      bool aux_pruned = false;
      AuxState next_aux[MAX_AUX];
      for (int a = 0; a < num_aux; ++a) {
      ...
      // [优化顺序] 先检�?prune_xc4 (Conj) - 6.10% 剪枝�?
      ...
      // NOTE: Search 4 不再使用 edge_prune1 (EC) �?prune1 (XC slot1) 做剪�?
      // 但仍需计算它们用于 depth==1 时验证解决状�?
      int index9_tmp = p_edge_move_ptr[arg_index9 + i];
      int edge_prune1_tmp =
          get_prune_ptr(edge_prune1, index9_tmp * 24 + index2_tmp);
      int prune1_tmp = get_prune_ptr(prune1, index1_tmp + index2_tmp);

      // NOTE: prune2/3/4 (Base �? 已移除，因为�?AuxState (Corner3) 完全覆盖
      int index4_tmp = p_corner_move_ptr[arg_index4 + i];
    ...
    for (const auto &task : tasks) {
      ...
      if (prune1_tmp == 0 && prune2_tmp == 0 && prune3_tmp == 0 &&
        ...
        int num_aux = setup_aux_pruners_for_search4(
            pslot1,                 // 参考槽�?(slot_k)
            slot2, slot3, slot4,    // Edge 槽位 (固定�?
            pslot2, pslot3, pslot4, // Corner 伪槽�?
...
struct PseudoPairSolverWrapper {
  ...
  std::string solve(const std::vector<int> &alg, const std::string &id) {
    // 将alg转换为字符串 (analyzer_compute需要scramble字符�?
  ...
  static void print_stats() {
    // NOTE: 所有剪枝统计已禁用 (优化完成)
    // 如需重新启用统计，请取消注释对应的统计变量、计数代码和 print_line 调用
    // Search 1: s1_prune1, s1_edge
    // Search 2: s2_xc2, s2_prune1, s2_edge (prune2 已移除，剪枝�?0%)
    // Search 3: s3_aux, s3_xc3, s3_prune1, s3_edge
  }
};
...
int main() {
  ...
  // (62.5% 通过�? 原因：表索引 cross_idx * 24 + corner_idx 包含绝对 Corner

  // NOTE: verify_pseudo_base_conj() 验证显示 pseudo_base_prune_tables
  // 也无法减�?(43.75% 通过�? 原因：C5/C6/C7_E* 表是用不�?Corner 基准生成�?
