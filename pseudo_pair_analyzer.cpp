...
STAT_DECL(s4_prune1); // S4: 基础剪枝表1
STAT_DECL(s4_edge);   // S4: Edge 剪枝表
STAT_DECL(s4_prune2); // S4: 基础剪枝表2
STAT_DECL(s4_prune3); // S4: 基础剪枝表3
...
struct XCrossSolver {
  ...
  // 以 pslot 视角追踪 Cross + Corner + 4个Edge
  ...
  // NOTE: prune2/prune3 已移除(剪枝表0%, 被 AuxState 完全覆盖)
    ...
    for (int k = 0; k < count_moves; ++k) {
      ...
      for (int a = 0; a < num_aux; ++a) {
        ...
        if (cur.move_mapper) {
          ...
          int m_rot = cur.move_mapper[m_conj];         // slot_k -> 规范表
...
struct PseudoPairSolverWrapper {
  ...
  static void print_stats() {
    ...
    // Search 2: s2_xc2, s2_prune1, s2_edge (prune2 已移除，剪枝表0%)
...
int main() {
  ...
  // 也无法减少(43.75% 通过率) 原因：C5/C6/C7_E* 表是用不同的Corner 基准生成的
