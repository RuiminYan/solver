...
struct PairSolver {
  ...
  static inline const unsigned char *s_p_pt_cross_C4C6E0E2 = nullptr;
  ...
  PairSolver() {
    ...
    p_pt_cross_C4C6E0E2 = s_p_pt_cross_C4C6E0E2;
  ...
  void get_conjugated_indices_full(const std::vector<int> &alg, int slot_k,
    ...
    for (int m : alg) {
      ...
      cur_corn = p_mt_corn[cur_corn + mc] * 18;
      cur_e0 = p_mt_edge[cur_e0 + mc] * 18;
  ...
  bool search_1(int i1, int i2, int i3, int depth, int prev, int slot_s1) {
    ...
    for (int k = 0; k < count; ++k) {
      ...
      S1_CHECK(s1_cross);
      if (get_prune(p_pt_cross_ins_C4, n1 + n2) >= depth) {
        S1_HIT(s1_cross);
        continue;
      }
      int n3 = p_mt_edge[i3 + mc];
      S1_CHECK(s1_pair);
      if (get_prune(p_pt_pair_C4E0, n3 * 24 + n2) >= depth) {
        S1_HIT(s1_pair);
        continue;
      }
      if (depth == 1) {
        if (get_prune(p_pt_cross_ins_C4, n1 + n2) == 0 &&
            get_prune(p_pt_pair_C4E0, n3 * 24 + n2) == 0)
          return true;
      } else {
  ...
  bool search_2(int im_p, int ic_p, int ie_p, int im_x, int ic_x, int ie_x,
    ...
    for (int k = 0; k < count; ++k) {
      ...
      int n_im_x = p_mt_edge4[im_x + mc_x], n_ic_x = p_mt_corn[ic_x + mc_x],
          n_ie_x = p_mt_edge[ie_x + mc_x];
      S2_CHECK(s2_xcross);
      if (get_prune(p_pt_cross_C4E0,
      ...
      S2_CHECK(s2_cross);
      if (get_prune(p_pt_cross_ins_C4, n_im_p + n_ic_p) >= depth) {
        S2_HIT(s2_cross);
        continue;
      }
      // 3. Pair (3.6%)
      ...
      S2_CHECK(s2_pair);
      if (get_prune(p_pt_pair_C4E0, n_ie_p * 24 + n_ic_p) >= depth) {
        S2_HIT(s2_pair);
        continue;
      }
      if (depth == 1) {
        if (get_prune(p_pt_pair_C4E0, n_ie_p * 24 + n_ic_p) == 0 &&
            get_prune(p_pt_cross_C4E0,
  ...
  int solve_3_group(const std::vector<int> &alg, int bound) {
    ...
    for (auto &p : pairs) {
      for (int tgt = 0; tgt < 4; ++tgt) {
        ...
        int h3 = get_prune(p_pt_cross_C4E0,
