...
bool MoveTableManager::loadAll() {
  ...
  if (!loadTable(mt_corn, "mt_corn.bin"))
    return false;
  if (!loadTable(mt_edge4, "mt_edge4.bin"))
    return false;
...
void MoveTableManager::genAllSequentially() {
  ...
  // 3. Edge4 Table (depends on edge_table, 17.4MB)
  if (!fileExists("mt_edge4.bin")) {
    loadMTEdge();
    GenerationTimer timer;
    std::cout << "Generating mt_edge4.bin..." << std::endl;
    mt_edge4 = createMultiMoveTable2(4, 2, 12, StateSpace::CROSS, mt_edge);
  ...
  if (!fileExists("mt_edge2.bin")) {
    ...
    mt_edge2 = createMultiMoveTable(2, 2, 12, StateSpace::EDGE2, mt_edge);
  ...
  if (!fileExists("mt_edge3.bin")) {
    ...
    mt_edge3 = createMultiMoveTable(3, 2, 12, StateSpace::EDGE3, mt_edge);
  ...
  if (!fileExists("mt_edge6.bin")) {
    ...
    mt_edge6 = createMultiMoveTable(6, 2, 12, StateSpace::EDGE6, mt_edge);
  ...
  if (!fileExists("mt_corn2.bin")) {
    ...
    mt_corn2 = createMultiMoveTable(2, 3, 8, StateSpace::CORNER2, mt_corn);
  ...
  if (!fileExists("mt_corn3.bin")) {
    ...
    // 3个角块 (8P3 * 3^3 = 9072)
    mt_corn3 = createMultiMoveTable(3, 3, 8, StateSpace::CORNER3, mt_corn);
  ...
  if (!fileExists("mt_ep4.bin")) {
    ...
    mt_ep4 = createMultiMoveTable(4, 1, 12, StateSpace::EP4, mt_ep1);
...
void MoveTableManager::genMTCorn() {
  if (loadTable(mt_corn, "mt_corn.bin")) {
...
void MoveTableManager::genMTEdge4() {
  if (loadTable(mt_edge4, "mt_edge4.bin")) {
    std::cout << "Loaded edge4 table from file." << std::endl;
    return;
  }
...
void MoveTableManager::genMTEdge2() {
  if (loadTable(mt_edge2, "mt_edge2.bin")) {
...
void MoveTableManager::genMTEdge3() {
  if (loadTable(mt_edge3, "mt_edge3.bin")) {
...
void MoveTableManager::genMTEdge6() {
  if (loadTable(mt_edge6, "mt_edge6.bin")) {
...
void MoveTableManager::genMTCorn2() {
  if (loadTable(mt_corn2, "mt_corn2.bin")) {
...
void MoveTableManager::genMTCorn3() {
  if (loadTable(mt_corn3, "mt_corn3.bin")) {
...
std::vector<int> createMTEdge() {
  std::vector<int> mt(StateSpace::EDGE * 18, -1);
  for (int i = 0; i < StateSpace::EDGE; ++i) {
    State s;
...
std::vector<int> createMTCorn() {
  std::vector<int> mt(StateSpace::CORNER * 18, -1);
  for (int i = 0; i < StateSpace::CORNER; ++i) {
    State s;
...
std::vector<int> createMTEO() {
  std::vector<int> mt(StateSpace::EO12 * 18, -1);
  for (int i = 0; i < StateSpace::EO12; ++i) {
